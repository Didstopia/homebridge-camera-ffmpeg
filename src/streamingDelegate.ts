...
import { CameraRecordingDelegate, EventTriggerOption, MediaContainerType, VideoCodecType } from 'hap-nodejs';
...
export class StreamingDelegate implements CameraStreamingDelegate {
    ...
    const options: CameraControllerOptions = {
      ...
      recording: /*!this.recording ? undefined : */ {
        options: {
          ...
          // motionService: true, // FIXME: This no longer exists in node-hap
          // doorbellService: true, // FIXME: This no longer exists in node-hap
        },
        delegate: this.recordingDelegate as CameraRecordingDelegate,
      },
    };
  ...
  private determineResolution(
    ...
    const filters: Array<string> =
      this.videoConfig.videoFilter?.split(",") || [];
  ...
  resizeSnapshot(snapshot: Buffer, resizeFilter?: string): Promise<Buffer> {
    return new Promise<Buffer>((resolve, reject) => {
      ...
      this.log.debug(
        "Resize command: " + this.videoProcessor + " " + ffmpegArgs,
        this.cameraName,
        this.videoConfig.debug
      );
      const ffmpeg = spawn(this.videoProcessor, ffmpegArgs.split(/\s+/), {
  ...
  async handleSnapshotRequest(
    ...
    try {
      ...
      const snapshot = await (this.snapshotPromise ||
        this.fetchSnapshot(resolution.snapFilter));
      ...
      const resized = await this.resizeSnapshot(
        snapshot,
        resolution.resizeFilter
      );
  ...
  private startStream(
    ...
    if (sessionInfo) {
      ...
      ffmpegArgs += // Video
        (this.videoConfig.mapvideo
          ? " -map " + this.videoConfig.mapvideo
          : " -an -sn -dn") +
        " -codec:v " +
        vcodec +
        " -pix_fmt yuv420p" +
      ...
      if (this.videoConfig.audio) {
        if (
          ...
          ffmpegArgs += // Audio
            (this.videoConfig.mapaudio
              ? " -map " + this.videoConfig.mapaudio
              : " -vn -sn -dn") +
            (request.audio.codec === AudioStreamingCodecType.OPUS
      ...
      if (this.videoConfig.returnAudioTarget) {
        const ffmpegReturnArgs =
          ...
          " -loglevel level" +
          (this.videoConfig.debugReturn ? "+verbose" : "");
